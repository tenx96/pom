...
import { BaseValidator, ValidationFunction } from "./BaseValidator";
...
> extends BaseValidator<ValueType> {
  ...
  private _schemaUpdaters: ((val: any) => void)[] = [];
    ...
    this._schemaUpdaters.push((val) => {
      {
        ...
        if (condition) {
          ...
          otherwise && otherwise(this._schemaUpdater(), val);
        }
      }
    });
    return this;
  ...
  private _generateDynamicValidationScehma = () => {
    this._dynamicValidationSchema = {};
    this._schemaUpdaters.forEach((updater) => updater(this._value)); //updaters added with when fn
